...
- 外部クラウドや分散ストレージとのシームレスな統合<br>
ローカル環境にMCPサーバーを置く場合、手元の端末からアクセスできるデータ範囲やストレージ容量に物理的な制限が生じる。しかし、MCPサーバーをリモート（クラウドやオンプレミスのサーバー群）に配置することで、オンプレミス、エッジ、パブリッククラウドなど、あらゆる場所に点在する外部オブジェクトストレージやファイルシステムに直接、高速かつ安定した接続を確保できる。

```
   Anthropic Cloud                  Client PC                              Enterprise Server
+-----------------------+       +---------------------+               +-------------------------+
|                       |       |    Claude Desktop   |               |        Kubernetes       |
|  +-----------------+  |       |                     |               |                         |
|  |     [ LLM ]     |<------------  [Your Prompt]    |  mcp-remote   |  +-------------------+  |
|  | (Claude 3.5 etc)|  |       |  +---------------+  |  (JSON/SSE)   |  | svc-mcp           |  |
...
- MCP ClientとMCP Serverの間で使われるJson形式
```
{
...
まとめると、MCPサーバーをローカル（Stdio）で実行するアプローチは、個人の生産性を極限まで高めることに適しており、おもにナレッジワーカーや個人のPC内で作業を完結させるエンジニアを対象としている。用途としては、手元のファイル整理などが主なユースケースとなる。<br>
一方で、SSE方式によるリモート実行は、企業のDX推進や社内データ基盤の構築といったエンタープライズ用途を対象とする。拠点やクラウドに散らばるペタバイト級の非構造化データの検索・統合や、負荷の高い社内APIとの連携を目的とするため、必然的に高性能なストレージや高スペックな処理基盤が必要となる。

ちなみにローカルで実行する場合を想定した動かし方を以下に示す。
```
Anthropic Cloud                               Client PC (Local Environment)
+-----------------------+       +-------------------------------------------------------+
|                       |       |    Claude Desktop                                     |
|  +-----------------+  |       |                                                       |
|  |     [ LLM ]     |<------------  [Your Prompt]                                      |
